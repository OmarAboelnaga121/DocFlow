...
import {
  ...
  validateBranchName,
} from './utils/url-validator.util';

@Injectable()
export class RepositoryService {
  private readonly openai: OpenAI;
  private readonly logger = new Logger(RepositoryService.name);
  private static readonly REPO_CACHE_TTL = 3600; // 1 hour TTL
  ...
  private async ingestRepository(repoId: string, url: string, branch: string) {
    ...
    try {
      ...
      this.logger.log(
        `[Repo ${repoId}] Ingestion started. Total text/code files found: ${filePaths.length}`,
      );
      ...
      for (let i = 0; i < filePaths.length; i++) {
        ...
        this.logger.log(
          `[Repo ${repoId}] [${i + 1}/${filePaths.length}] Embedding file: ${relativePath}`,
        );
      ...
      this.logger.log(
        `[Repo ${repoId}] Embedding complete. Transitioning to ANALYZING...`,
      );
      await this.prisma.repo.update({
        where: { id: repoId },
  ...
  async syncRepo(repoId: string, userId: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Repository sync or ingestion is already in progress',
      );
    }

    // Step 3: Check if the repository already has the latest commit
    const remoteCommitHash = await this.getRemoteLatestCommit(
      repo.url,
      repo.branch,
    );
    ...
    if (
      ...
      remoteCommitHash === repo.latestCommitHash
    ) {
      return {
  ...
  private async performSyncRepository(
    ...
    try {
      ...
      // Transition to CLONING state
      this.logger.log(
        `[Repo ${repoId}] Starting shallow clone from ${url} (branch: ${branch})...`,
      );
      await this.prisma.repo.update({
        where: { id: repoId },
        data: { status: 'CLONING' },
      ...
      const latestCommitHash =
        knownCommitHash || (await gitRepo.revparse(['HEAD'])).trim();
      ...
      this.logger.log(
        `[Repo ${repoId}] Diff results: ${newFiles.length} new, ${updatedFiles.length} updated, ${deletedFileIds.length} deleted.`,
      );
      ...
      if (deletedFileIds.length > 0) {
        this.logger.log(
          `[Repo ${repoId}] Removing ${deletedFileIds.length} deleted files and stale chunks...`,
        );
      ...
      for (let i = 0; i < updatedFiles.length; i++) {
        ...
        this.logger.log(
          `[Repo ${repoId}] [${i + 1}/${updatedFiles.length}] Re-embedding updated file: ${updated.relativePath}`,
        );
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        this.logger.log(
          `[Repo ${repoId}] [${i + 1}/${newFiles.length}] Embedding new file: ${newFile.relativePath}`,
        );
      ...
      // Transition to ANALYZING state
      this.logger.log(
        `[Repo ${repoId}] Resync embeddings complete. Transitioning to ANALYZING...`,
      );
      await this.prisma.repo.update({
        where: { id: repoId },
        data: { status: 'ANALYZING' },
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentLength >= chunkSize || i === lines.length - 1) {
        ...
        if (overlapLinesCount > 0 && i < lines.length - 1) {
          ...
          currentLength = currentLines.reduce(
  ...
  async getRepoById(repoId: string, userId?: string) {
    const repo = await this.redis.getOrSet(
      `repo:${repoId}`,
      RepositoryService.REPO_CACHE_TTL,
      async () => {
        const found = await this.prisma.repo.findUnique({
          where: { id: repoId },
          include: {
            files: true,
            chats: {
  ...
  async deleteRepo(repoId: string, userId?: string) {
    ...
    if (fs.existsSync(clonePath)) {
      try {
        ...
        console.warn(
          `Failed to clean up temp clone directory ${clonePath}:`,
          err,
        );
      }
    }

    // 9. Invalidate cached repository and the user's repository list
    await this.redis.invalidateCache([
      `repo:${repoId}`,
      `user-repos:${repo.userId}`,
    ]);
